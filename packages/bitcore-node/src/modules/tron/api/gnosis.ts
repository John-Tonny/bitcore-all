...
export class GnosisApi {
  ...
  private TRX_MULTISIG_TX_PROPOSAL_EXPIRE_TIME = 48 * 3600 * 1000;

  async multisigFor(network: string, address: string) {
    const { web3 } = await TRX.getWeb3(network);
  ...
  async getMultisigContractInstantiationInfo(
    ...
    const networkConfig = Config.chainConfig({ chain: 'TRX', network });
    const { gnosisFactory = this.gnosisFactories[network] } = networkConfig;
    let query = { chain: 'TRX', network, txid: txId };
  ...
  async getMultisigTxpsInfo(network: string, multisigContractAddress: string): Promise<Partial<Transaction>[]> {
    ...
    const time = Math.floor(Date.now()) - this.TRX_MULTISIG_TX_PROPOSAL_EXPIRE_TIME;
    const [block] = await TrxBlockStorage.collection
      .find({
        chain: 'TRX',
        network,
  ...
  async getMultisigTrxInfo(network: string, multisigContractAddress: string) {
  ...
  async streamGnosisWalletTransactions(params: { multisigContractAddress: string } & StreamWalletTransactionsParams) {
    ...
    const { web3 } = await TRX.getWeb3(network);
    const transactionQuery = TRX.getWalletTransactionQuery(params);
    ...
    const trxTransactionTransform = new TrxListTransactionsStream([multisigContractAddress, args.tokenAddress]);
    ...
    if (multisigContractAddress) {
      const trxMultisigTransform = new TrxMultisigRelatedFilterTransform(
        web3,
        multisigContractAddress,
        args.tokenAddress
      );
      transactionStream = transactionStream.pipe(trxMultisigTransform);
    }

    transactionStream
      .pipe(populateReceipt)
      .pipe(trxTransactionTransform)
      .pipe(res);
  }
}
