...
export class TRXStateProvider extends InternalStateProvider implements IChainStateService {
  config: any;
  ...
  async getWeb3(network: string): Promise<{ rpc: CryptoRpc; web3: Web3 }> {
    try {
      if (TRXStateProvider.rpcs[network]) {
        await TRXStateProvider.rpcs[network].web3.trx.getCurrentBlock();
      }
    } catch (e) {
      delete TRXStateProvider.rpcs[network];
    }
    if (!TRXStateProvider.rpcs[network]) {
      ...
      TRXStateProvider.rpcs[network] = { rpc, web3: rpc.web3 };
    }
    return TRXStateProvider.rpcs[network];
  }

  async erc20For(network: string, address: string) {
    const { web3 } = await this.getWeb3(network);
    const contract = await web3.contract.at(address);
    return contract;
  }

  async getERC20TokenInfo(network: string, tokenAddress: string) {
    const token = await TRX.erc20For(network, tokenAddress);
  ...
  async getFee(params) {
    ...
    return CacheStorage.getGlobalOrRefresh(
      cacheKey,
      async () => {
        const txs = await TrxTransactionStorage.collection
    ...
    const balances = await CacheStorage.getGlobalOrRefresh(
      ...
      async () => {
        if (tokenAddress) {
          ...
          const unbalance = await web3.trx.getUnconfirmedBalance(address);
          const numberBalance = Number(balance);
          const unnumberBalance = Number(unbalance);
          return { confirmed: numberBalance, unconfirmed: unnumberBalance, balance: numberBalance };
        }
      },
  ...
  async getLocalTip({ chain, network }) {
    return TrxBlockStorage.getLocalTip({ chain, network });
  }

  async getReceipt(network: string, txid: string) {
    const { web3 } = await this.getWeb3(network);
    return web3.trx.getTransactionInfo(txid);
  }

  async populateReceipt(tx: MongoBound<ITrxTransaction>) {
    if (!tx.internal) {
      const receipt = await this.getReceipt(tx.network, tx.txid);
      if (receipt && receipt.InternalTransaction && receipt.InternalTransaction.length > 0) {
        const fee = receipt.fee;
        await TrxTransactionStorage.collection.updateOne({ _id: tx._id }, { $set: { receipt, fee } });
        tx.internal = receipt.InternalTransaction;
        tx.fee = fee;
      }
    }
    return tx;
  ...
  async getTransaction(params: StreamTransactionParams) {
    try {
      ...
      let found = await TrxTransactionStorage.collection.findOne(query);
      if (found) {
        let confirmations = 0;
        ...
        const convertedTx = TrxTransactionStorage._apiTransform(found, { object: true }) as TrxTransactionJSON;
  ...
  async broadcastTransaction(params: BroadcastTransactionParams) {
    ...
    for (const rawTx of rawTxs) {
      const result = await web3.trx.sendRawTransaction(rawTx);
  ...
  async streamAddressTransactions(params: StreamAddressUtxosParams) {
    ...
    if (!args.tokenAddress) {
      ...
      Storage.apiStreamingFind(TrxTransactionStorage, query, { limit, since, paging: '_id' }, req!, res!);
    } else {
      try {
  ...
  async streamTransactions(params: StreamTransactionsParams) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    return Storage.apiStreamingFind(TrxTransactionStorage, query, args, req, res, t => {
      let confirmations = 0;
      if (t.blockHeight !== undefined && t.blockHeight >= 0) {
        confirmations = tipHeight - t.blockHeight + 1;
      }
      const convertedTx = TrxTransactionStorage._apiTransform(t, { object: true }) as Partial<ITransaction>;
  ...
  async streamWalletTransactions(params: StreamWalletTransactionsParams) {
    ...
    transactionStream = TrxTransactionStorage.collection
      .find(query)
  ...
  async getBlocks(params: GetBlockParams) {
    ...
    let cursor = TrxBlockStorage.collection.find(query, options).addCursorFlag('noCursorTimeout', true);
    if (options.sort) {
    ...
    const tip = await this.getLocalTip(params);
    const tipHeight = tip ? tip.height : 0;
    const blockTransform = (b: ITrxBlock) => {
      let confirmations = 0;
      if (b.height && b.height >= 0) {
        confirmations = tipHeight - b.height + 1;
      }
      const convertedBlock = TrxBlockStorage._apiTransform(b, { object: true }) as ITrxBlock;
  ...
  async updateWallet(params: UpdateWalletParams) {
    ...
    for (let addressBatch of addressBatches) {
      ...
      await TrxTransactionStorage.collection.updateMany(
        {
          $or: [
...
export const TRX = new TRXStateProvider();
