...
import { TrxP2pWorker } from './p2p';

export class TrxVerificationPeer extends TrxP2pWorker implements IVerificationPeer {
  prevBlockNum = 0;
  prevHash = '';
  nextBlockHash = '';
  deepScan = false;
  ...
  async resync(start: number, end: number) {
    ...
    while (currentHeight <= end) {
      ...
      const nextBlock = await TrxBlockStorage.collection.findOne({ chain, network, previousBlockHash: block.blockID });
      if (nextBlock) {
