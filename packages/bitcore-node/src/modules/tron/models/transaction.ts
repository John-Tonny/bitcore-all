...
@LoggifyClass
export class TrxTransactionModel extends BaseTransaction<ITrxTransaction> {
  ...
  async onConnect() {
    ...
    this.collection.createIndex(
      ...
      {
        background: true,
        partialFilterExpression: { chain: 'TRX', 'abiType.type': 'ERC20', 'abiType.name': 'transfer' }
      }
    );
    this.collection.createIndex(
      { chain: 1, network: 1, 'internal.caller_address': 1 },
      {
        background: true,
        sparse: true
      }
  ...
  async batchImport(params: {
    ...
    if (params.height < SpentHeightIndicators.minimum) {
      for (let op of txOps) {
        ...
        const tx = { ...op.updateOne.update.$set, ...filter } as ITrxTransaction;
  ...
  async addTransactions(params: {
    ...
    if (parentChain && forkHeight && height < forkHeight) {
      const parentTxs = await TrxTransactionStorage.collection
      ...
      return Promise.all(
        params.txs.map(async (tx: ITrxTransaction) => {
  ...
  getTransactions(params: { query: any; options: StreamingFindOptions<ITrxTransaction> }) {
  ...
  _apiTransform(
    tx: ITrxTransaction | Partial<MongoBound<ITrxTransaction>>,
    options?: TransformOptions
  ): TrxTransactionJSON | string {
    const dataStr = `0x${tx.data!.toString('hex')}`;
    const decodedData = this.abiDecode(dataStr);

    const transaction: TrxTransactionJSON = {
      txid: tx.txid || '',
      network: tx.network || '',
      chain: tx.chain || '',
...
export let TrxTransactionStorage = new TrxTransactionModel();
